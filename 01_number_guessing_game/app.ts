...
const UserNumber = async (number_range: number) => {
    let value: number = 0
    while (true) {
        const input = await inquirer.prompt([
            {
...
const GuessedNumber = (difficulty: string) => {
...
while (true) {
    ...
    let difficulty: string = await DifficultyLevel()
    ...
    let value: boolean = await input['\x1B[38;2;255;255;160mDo You Want To Play Again ?\x1B[39m']
    if (!value) {
